...
if __name__ == '__main__':
    ...
    get.add_argument("-i", "--ip-address", help="IP Address of server", default="127.0.0.1")
    get.add_argument("-p", "--port", help="Port to the server", required=True, type=int)
    get.add_argument("-s", "--store", help="File to store, defaults to file's base name", default=None)

    post.add_argument("-f", "--file", help="File to post", required=True)
    post.add_argument("-i", "--ip-address", help="IP Address of server", default="127.0.0.1")
    post.add_argument("-p", "--port", help="Port to the server", required=True, type=int)
    post.add_argument("-s", "--store", help="Path for the file to be stored on server, defaults to file's basename",
                      default=None)

    new.add_argument("-d", "--directory", help="Directory Name", required=True)
    new.add_argument("-i", "--ip-address", help="IP Address of server", default="127.0.0.1")
    new.add_argument("-p", "--port", help="Port to the server", required=True, type=int)

    dir_lister.add_argument("-i", "--ip-address", help="IP Address of server", default="127.0.0.1")
    dir_lister.add_argument("-p", "--port", help="Port to the server", required=True, type=int)
    dir_lister.add_argument("-d", "--directory", help="Directory to list", required=True)

    host.add_argument("-i", "--ip-address", help="IP Address of server", default="127.0.0.1")
    host.add_argument("-p", "--port", help="Port to the server", required=True, type=int)
    host.add_argument("-e", "--post-directory", help="Allow the post of users to be limited to this directory",
                      default=None)
    ...
    try:
        if args.command == 'host':
            ...
            bdtpfserv.host(args.directory, (args.ip_address, args.port), args.post_directory)
            ...
            directory_list = bdtpfserv.listdir(args.directory, (args.ip_address, args.port))
